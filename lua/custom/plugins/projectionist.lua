...
local function ensure_elixir_module_transform()
  if vim.g.projectionist_transformations == nil then
    vim.g.projectionist_transformations = vim.empty_dict()
  end

  _G.ProjectionistElixirModuleTransform = function(input)
    return input:gsub('(%.%l)', string.upper)
  end

  vim.cmd [[
    function! g:projectionist_transformations.elixir_module(input, o) abort
      return v:lua.ProjectionistElixirModuleTransform(a:input)
    endfunction
  ]]
end

local function extend_heuristics()
  local existing_heuristics = vim.g.projectionist_heuristics or {}

  vim.g.projectionist_heuristics = vim.tbl_deep_extend('keep', existing_heuristics, heuristics)
end

return {
  'tpope/vim-projectionist',
  init = function()
    ensure_elixir_module_transform()
    extend_heuristics()
  end,
}
