return {
  ...
  init = function()
    ...
    -- quickfix
    map('n', prefix .. 'q', function() haunt.to_quickfix() end,                            { desc = 'Hauntings -> QF (buffer)' })
    map('n', prefix .. 'Q', function() haunt.to_quickfix { current_buffer = true } end,    { desc = 'Hauntings -> QF (all)' })

    -- yank
    map('n', prefix .. 'y', function() haunt.yank_locations { current_buffer = true } end, { desc = 'Hauntings -> Clipboard (buffer)' })
    map('n', prefix .. 'Y', function() haunt.yank_locations() end,                         { desc = 'Hauntings -> Clipboard (all)' })
  end,
}
